...
@Controller
public class SupplierCommentReplySVController extends BaseController{
	public final String prefix="/supplier_comment_reply_";
	@Autowired public SupplierCommentReplyMapper dao;
	@ResponseBody @RequestMapping(prefix+"list.do")//商家评论细节列表ysc
	...
	@ResponseBody @RequestMapping(prefix+"detail.do")//商家评论细节详情ysc
	public Map<String,Object> detail(HttpServletRequest request) throws Exception {
		...
		String json=request.getParameter("json");try {
			if(StringUtils.isBlank(json)){
				...
				VSupplierCommentReply bean=dao.getBeanById(Integer.parseInt(id.trim()));
