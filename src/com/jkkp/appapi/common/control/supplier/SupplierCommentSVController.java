...
@Controller
public class SupplierCommentSVController extends BaseController{
	public final String prefix="/supplier_comment_";
	@Autowired public SupplierCommentMapper dao;
	@ResponseBody @RequestMapping(prefix+"list.do")//商家评论列表ysc
	...
	@ResponseBody @RequestMapping(prefix+"detail.do")//商家评论详情ysc
	public Map<String,Object> detail(HttpServletRequest request) throws Exception {
		...
		String json=request.getParameter("json");try {
			if(StringUtils.isBlank(json)){
				...
				VSupplierComment bean=dao.getBeanById(Integer.parseInt(id.trim()));
