...
@Controller
public class SupervisorSVController extends BaseController{
	public static final String head="/supervisor_";
	@Autowired public SupervisorMapper dao;
	@ResponseBody @RequestMapping(head+"list.do")//监理报告列表
	...
	@ResponseBody @RequestMapping(head+"detail.do")//监理报告详情
	public Map<String,Object> detail(HttpServletRequest request) throws Exception {
		...
		String json=request.getParameter("json");try {
			if(StringUtils.isBlank(json)){
				...
				VSupervisor bean=dao.getBeanById(Integer.parseInt(id));
	...
	public String getAndByJSON(HttpServletRequest request,JSONObject json){
		...
		and+=BaseTools.getAndByJson(json,"ysjd_id","ysjd_id", "1","1");
