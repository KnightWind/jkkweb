		 ...
		 <div class="tools white border mb10 sp10 clr">
				...
				<ul>					
					<li>
						...
						<span class="sp"><strong class="auto">商家：</strong><input type="text" class="text" name="snickName" id=""></span>
					</li>					
					<li>
						<strong>状态：</strong>
					    <span class="sp radio">
			...
			<div class="page_tab_box">
                <div class="page_tab">
                    <div class="pageTab">
                        <span class="cur"><a href="${base}/supplierComplain/index.xhtml?pid=${pid}&mid=${mid}">全部</a></span>
                        <span ><a href="${base}/supplierComplain/index.xhtml?pid=${pid}&mid=${mid}&type=2">商家投诉业主</a></span>
                        <span ><a href="${base}/supplierComplain/index.xhtml?pid=${pid}&mid=${mid}&type=1">业主投诉商家</a></span>                                              
                    </div>
                </div>
            </div>
			...
			<table class="format">
				...
				<tbody>
					...
					<tr>						
						...
						<td>
	                        [#if item.status==1]
							    <span class="btn"><a href="detail.xhtml?pid=${pid}&mid=${mid}&id=${item.id}&cid=${item.id}">查看</a></span>				
							    <span style="padding: 4px 12px; background: #cccccc; border-radius: 3px;" >关闭</span>		
							[#else]			
							    <span class="btn"><a href="detail.xhtml?pid=${pid}&mid=${mid}&id=${item.id}&cid=${item.id}">跟进</a></span>				
							    <span class="btn del" rel="${base}/supplierComplain/close.do?id=${item.id}">关闭</span>
							[/#if]
                        </td>
					</tr>
					[/#list]
...
<%for(i=0;i<result.elements.length;i++){%>
	<tr>
		...
		<% if(result.elements[i].status==1){%>
	        <span class="btn"><a href="detail.xhtml?pid=${pid}&mid=${mid}&id=<%=result.elements[i].id%>&cid=<%=result.elements[i].id%>">查看</a></span>				
			<span style="padding: 4px 12px; background: #cccccc; border-radius: 3px;" >关闭</span>	
		<%}%>
		   
		<% if(result.elements[i].status==2){%>
	        <span class="btn"><a href="detail.xhtml?pid=${pid}&mid=${mid}&id=<%=result.elements[i].id%>&cid=<%=result.elements[i].id%>">跟进</a></span>				
			<span class="btn del" rel="${base}/supplierComplain/close.do?id=<%=result.elements[i].id%>">关闭</span>
		<%}%>  
		</td>
	</tr>
<%}%>
...
$(function(){
	$('.ui-paging').page({
		url:'${base}/supplierComplain/pagination.do',
