	...
	<div class="conWrap fl">
		...
		<div class="tools white border mb10 sp10 clr">
			...
			<div class="page_tab_box">
                <div class="page_tab">
                    <div class="pageTab">
                        <span class="cur"><a href="${base}/staffComment/designReplyList.xhtml?id=${staffId}&state=-1&mid=${mid}&pid=${pid}&returnUrl=staff_design_replyList&status=0">全部状态</a></span>
                        <span ><a href="${base}/staffComment/designReplyList.xhtml?id=${staffId}&state=1&mid=${mid}&pid=${pid}&returnUrl=staff_design_replyList&status=1">发布状态</a></span>
                        <span ><a href="${base}/staffComment/designReplyList.xhtml?id=${staffId}&state=0&mid=${mid}&pid=${pid}&returnUrl=staff_design_replyList&status=2">屏蔽状态</a></span>
                    </div>
                </div>
            </div>	
			<table class="format" id="table_detail">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							[#if item.status == 0]
								<span class="btn del" rel="${base}/staffComment/operationApproval.do?flag=true&id=${item.id}">解除</span>
							[#else]
								<span class="btn del" rel="${base}/staffComment/operationApproval.do?flag=false&id=${item.id}">屏蔽</span>
							[/#if]
	...
	<tr>
		...
		<td> 
			<% if(result.elements[i].status == 0) {%>
				<span class="btn del" rel="${base}/staffComment/operationApproval.do?flag=true&id=<%=result.elements[i].id%>">解除</span>
			<%}else{%>
				<span class="btn del" rel="${base}/staffComment/operationApproval.do?flag=false&id=<%=result.elements[i].id%>" >屏蔽</span>
			<%}%>
...
$(function(){
	$('.ui-paging').page({
		url:'${base}/staffComment/designReplyPagination.do?id=${staffId}&state=${state}',
		deal:{'click':'.search'},
		send:function(){
			var data={
					pageSize:10,
