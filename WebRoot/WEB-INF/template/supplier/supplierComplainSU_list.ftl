...
<body>
			...
			<table class="format">
				...
				<tbody>
					...
					<tr>						
						...
						<td>
						<span class="btn"><a href="detailSU.xhtml?id=${item.id}&cid=${item.id}">查看</a></span>				
                        </td>
					</tr>
					[/#list]
				</tbody>
   ...
   [#include "/common/supplierMenu.ftl"]
    ...
    <%for(i=0;i<result.elements.length;i++){%>
		...
		<td>
	       <span class="btn"><a href="detailSU.xhtml?id=<%=result.elements[i].id%>&cid=<%=result.elements[i].id%>">查看</a></span>				
		</td>
	 </tr>
   <%}%>
</script>
